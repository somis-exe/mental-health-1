...
/** Ordinal score (higher = better appetite) so appetite can be plotted like mood/sleep. */
export const APPETITE_SCORE: Record<string, number> = {
  [APPETITE[0]]: 3,
  [APPETITE[1]]: 2,
  [APPETITE[2]]: 1,
}

/** Ordinal score (higher = more activity) so exercise can be plotted like mood/sleep. */
export const EXERCISE_SCORE: Record<string, number> = {
  [EXERCISE[0]]: 0,
  [EXERCISE[1]]: 1,
  [EXERCISE[2]]: 2,
}
...
export function metricDayValue(key: ReportMetricKey, record: DailyRecord): number | null {
  switch (key) {
    case 'mood':
      return averageMood(record)
    case 'sleep':
      return record.sleepHours
    case 'appetite':
      return record.appetite !== null ? (APPETITE_SCORE[record.appetite] ?? null) : null
    case 'exercise':
      return record.exercise !== null ? (EXERCISE_SCORE[record.exercise] ?? null) : null
    case 'bath':
